{{
  config(
    materialized='table',
    description='Comprehensive data quality analysis identifying issues and their business impact'
  )
}}
...
WITH base_stats AS (
    SELECT 
        ...
        SUM(total_emissions) AS total_emissions_all,
...
quality_issues AS (
    ...
    SELECT 
        ...
        0 AS emissions_impact,  -- No emissions to calculate for null values
    ...
    SELECT 
        ...
        ROUND(COUNT(*) * 100.0 / (SELECT total_records FROM base_stats), 2) AS records_impact_pct,
        ROUND(COUNT(DISTINCT domain) * 100.0 / (SELECT total_domains FROM base_stats), 2) AS domains_impact_pct,
        ROUND(SUM(ABS(total_emissions - calculated_total_emissions)), 6) AS emissions_impact,
    ...
    SELECT 
        ...
        ROUND(COUNT(*) * 100.0 / (SELECT total_records FROM base_stats), 2) AS records_impact_pct,
        ROUND(COUNT(DISTINCT domain) * 100.0 / (SELECT total_domains FROM base_stats), 2) AS domains_impact_pct,
        ROUND(SUM(ABS(total_emissions)), 6) AS emissions_impact,
        CASE 
    ...
    SELECT 
        ...
        ROUND(COUNT(*) * 100.0 / (SELECT total_records FROM base_stats), 2) AS records_impact_pct,
        ROUND(COUNT(DISTINCT domain) * 100.0 / (SELECT total_domains FROM base_stats), 2) AS domains_impact_pct,
        ROUND(SUM(total_emissions), 6) AS emissions_impact,
        'MEDIUM' AS severity_level,
    ...
    SELECT 
        'unknown_domain_coverage' AS issue_type,
        'Unknown or Missing Domain Coverage Information' AS issue_description,
        COUNT(*) AS affected_records,
        COUNT(DISTINCT domain) AS affected_domains,
        ROUND(COUNT(*) * 100.0 / (SELECT total_records FROM base_stats), 2) AS records_impact_pct,
        ROUND(COUNT(DISTINCT domain) * 100.0 / (SELECT total_domains FROM base_stats), 2) AS domains_impact_pct,
        ...
        CURRENT_TIMESTAMP() AS analyzed_at
    FROM {{ ref('stg_advertising_emissions') }}
    WHERE domain_coverage = 'unknown'
),

domain_quality_impact AS (
    SELECT 
        domain,
        COUNT(*) AS total_records,
        SUM(CASE WHEN has_null_emissions THEN 1 ELSE 0 END) AS null_emissions_count,
        SUM(CASE WHEN has_emissions_mismatch THEN 1 ELSE 0 END) AS mismatch_count,
        SUM(CASE WHEN has_extreme_emissions THEN 1 ELSE 0 END) AS extreme_count,
        AVG(data_quality_score) AS avg_quality_score,
        SUM(total_emissions) AS total_emissions,
        ROUND(
            (SUM(CASE WHEN has_null_emissions THEN 1 ELSE 0 END) + 
             SUM(CASE WHEN has_emissions_mismatch THEN 1 ELSE 0 END) + 
             SUM(CASE WHEN has_extreme_emissions THEN 1 ELSE 0 END)) * 100.0 / COUNT(*),
            2
        ) AS issue_rate_pct,
        CASE 
            WHEN AVG(data_quality_score) >= 90 THEN 'Excellent'
            WHEN AVG(data_quality_score) >= 80 THEN 'Good'
            WHEN AVG(data_quality_score) >= 70 THEN 'Fair'
            WHEN AVG(data_quality_score) >= 60 THEN 'Poor'
            ELSE 'Very Poor'
...
quality_summary AS (
    SELECT 
        ...
        (SELECT total_records FROM base_stats) AS total_records_analyzed,
        (SELECT total_domains FROM base_stats) AS total_domains_analyzed,
        (SELECT total_days FROM base_stats) AS total_days_analyzed,
        (SELECT ROUND(overall_quality_score, 2) FROM base_stats) AS overall_quality_score,
        
        -- Issue summary
        COUNT(*) AS total_issue_types,
        SUM(affected_records) AS total_affected_records,
        SUM(DISTINCT affected_domains) AS total_affected_domains,
        ROUND(SUM(affected_records) * 100.0 / (SELECT total_records FROM base_stats), 2) AS overall_impact_pct,
        
        -- Emissions impact
        ROUND(SUM(emissions_impact), 6) AS total_emissions_at_risk,
        ROUND(SUM(emissions_impact) * 100.0 / (SELECT total_emissions_all FROM base_stats), 2) AS emissions_impact_pct,
...
recommendations AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN issue_type = 'unknown_domain_coverage' THEN 'Enhance domain coverage tracking. Update data dictionary.'
...
SELECT 
    ...
    (SELECT overall_quality_score FROM quality_summary) AS baseline_quality_score,
    (SELECT total_records_analyzed FROM quality_summary) AS total_records_context,
